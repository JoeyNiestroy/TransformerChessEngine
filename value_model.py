...
class Chess_Model_Value(nn.Module):
    ...
    def forward(self, input1, input2, mask):
        ...
        # output1_temp =self.action_ff(encoded_action)

        output1 = None#self.output_layer1(torch.flatten(encoded, start_dim=1))
        ...
        # output2 = self.output_layer2(torch.flatten(encoded , start_dim=1))
        action_values_output = None#output2.view(-1, 1968, 63)
        ...
        return output1 , action_values_output  , output3 ,encoded[:,-1,:], values
...
if __name__ == '__main__':
    model = Chess_Model_Value(dim = 256, num_layers = 8, nheads = 4)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # model.load_state_dict(torch.load('/media/joey/Elements/Critic_basic_small.pth'))
    ...
    optimizer =  optim.AdamW(model.parameters(), lr = 1e-5,betas=(0.9, 0.999) , weight_decay = 1e-2)    #optim.SGD(model.parameters(), lr = 1e-3, momentum=.9)#optim.Adam(model.parameters(), lr = 5e-4,betas=(0.9, 0.999))  # Feel free to use any optimizer
#optim.Adagrad(model.parameters(), lr = 1e-3)
    ...
    for epoch in range(15,27): #119
            ...
            torch.save(model.state_dict(), '/media/joey/Elements/Critic_basic_medium_'+str(epoch)+'.pth')
    torch.save(model.state_dict(), '/media/joey/Elements/Critic_basic_medium.pth')