...
class Chess_Model_Actor(nn.Module):
    def __init__(self, dim = 2048, num_layers = 24, nheads = 4):
        ...
        self.output_layer1 = nn.Linear(dim,1968)  # Output layer 1
    ...
    def forward(self, input1, input2, mask):
        ...
        output1 = self.output_layer1(encoded) #torch.flatten(encoded, start_dim=1)
        mask_unsqueezed = mask.unsqueeze(1)  # Now shape is (batch_size, 1, 4097)
        ...
        output1_final = output1.masked_fill(mask_repeated, _MASKING_VALUE)
        ...
        values = self.reward_layer(self.tanh(torch.flatten(encoded, start_dim=1)))
...
if __name__ == '__main__':
    model = Chess_Model_Actor(dim = 128, num_layers= 8, nheads = 4)
    ...
    criterion3 = CornLoss(32)#nn.MSELoss() #nn.CrossEntropyLoss(ignore_index=32) #nn.MSELoss() #
    criterion4 = nn.CrossEntropyLoss(ignore_index=13)
    optimizer =  optim.AdamW(model.parameters(), lr = 1e-4,betas=(0.9, 0.95), eps = 1e-5, weight_decay= 1e-4)    #optim.SGD(model.parameters(), lr = 1e-3, momentum=.9)#optim.Adam(model.parameters(), lr = 5e-4,betas=(0.9, 0.999))  # Feel free to use any optimizer
    ...
    for epoch in range(15,50): #119
            ...
            for batch_idx, (data, target, mask, targets_move, values) in enumerate(data_loader):
                ...
                if batch_size == 1:
                    ...
                    output1 = output1.view(-1, 1968)  # Flatten to [128*65, 4097]
                    ...
                    # loss3 = criterion3(predicted_values, final_values.view(-1)) #values
                    # penalty = (1 - torch.abs(predicted_values).mean())
                    loss3_float = 1#loss3.item()
            ...
            torch.save(model.state_dict(), '/media/joey/Elements/Actor_medium_'+str(epoch)+'.pth')
    torch.save(model.state_dict(), '/media/joey/Elements/Actor_medium.pth')